...
class _WatchedList extends ListBase<BoardArtefact> implements List<BoardArtefact> {
  ...
  set length(int newLength) {
    ...
    if (newLength != oldLength) {
      debugPrint('[TalkingmatController] _WatchedList - length changed from $oldLength to $newLength');
      if (newLength == 0 && oldLength > 0) {
        debugPrint('[TalkingmatController] WARNING: List cleared via length setter!');
        try {
  ...
  void clear() {
    final int oldLength = _list.length;
    debugPrint('[${_controller._instanceId}] _WatchedList - clear() called! List had $oldLength items');
    if (oldLength > 0) {
      debugPrint('[${_controller._instanceId}] WARNING: List cleared via clear()! Stack trace:');
      try {
        throw Exception('Stack trace for debugging');
      } catch (e, stackTrace) {
  ...
  bool remove(Object? value) {
    final bool result = _list.remove(value);
      if (result) {
        debugPrint('[TalkingmatController] _WatchedList - remove() called, item removed');
        _controller._notifyListeners();
      }
    return result;
  }
  ...
  void removeWhere(bool Function(BoardArtefact) test) {
    ...
    if (removed > 0) {
      debugPrint('[TalkingmatController] _WatchedList - removeWhere() removed $removed item(s)');
      _controller._notifyListeners();
    }
  }
  ...
  @override
  void sort([int Function(BoardArtefact, BoardArtefact)? compare]) => _list.sort(compare);
}
...
class TalkingmatController extends ValueNotifier<List<BoardArtefact>> {
  ...
  TalkingmatController({List<BoardArtefact>? initialArtifacts, this.onArtefactAdded})
    ...
    debugPrint('[$_instanceId] CREATED with ${initialArtifacts?.length ?? 0} initial artifacts');
  ...
  set value(List<BoardArtefact> newValue) {
    ...
    debugPrint('[$_instanceId] value SETTER CALLED - oldLength: $oldLength, newLength: $newLength');
    
    if (oldLength != newLength) {
      debugPrint('[$_instanceId] value SETTER - Count changed from $oldLength to $newLength');
      
      if (oldLength > newLength && newLength == 0) {
        debugPrint('[$_instanceId] WARNING: Controller value cleared from $oldLength to 0!');
        ...
        debugPrint('[$_instanceId] value SETTER - Artifacts removed (${oldLength - newLength} removed)');
      } else {
        debugPrint('[$_instanceId] value SETTER - Artifacts added (${newLength - oldLength} added)');
      }
    } else if (oldLength == 0 && newLength == 0) {
      debugPrint('[$_instanceId] WARNING: Setting empty list to empty list - stack trace:');
      try {
        throw Exception('Stack trace for debugging');
      } catch (e, stackTrace) {
        debugPrint('[$_instanceId] Stack trace: $stackTrace');
  ...
  void addArtifact(BoardArtefact artefact) {
    ...
    debugPrint('[$_instanceId] Adding artifact ID:$artifactId, position: ${artefact.position}, total count: ${value.length + 1}');
    value.add(artefact);
